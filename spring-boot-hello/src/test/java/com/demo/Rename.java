...
public class Rename {
    ...
    static String dir = "D:\\soft\\JiJiDown\\Download\\activiti";//文件所在路径，所有文件的根目录，记得修改为你电脑上的文件所在路径
            ...
            if (null == fileArr || fileArr.length == 0) {
                ...
                for (File file : fileArr) {
                    if (file.isDirectory()) {//是文件夹，继续递归，如果需要重命名文件夹，这里可以做处理
                        ...
                        extracted(fileName, parentPath, file);
                    }
                }
            }
        } else {
    ...
    private static void extracted(String fileName, File parentPath, File file) {
        File newDir;
        String newName;
        String name = FileNameUtil.mainName(file);;
        String type = FileNameUtil.extName(file);
        newName = name.substring(0,3)+"."+type;
